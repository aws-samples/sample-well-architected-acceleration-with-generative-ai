...
def bedrock_prompt(wafr_lens, pillar, questions, kb_id, document_content=None, wafr_reference_bucket = None):    
    ...
    4) For each question, 
        a) Provide which Best practices from the specified pillar have been followed, including the best practice titles and IDs from the respective pillar guidance for the question. List them under the 'Best practices followed' section. Example: REL01-BP03 Accommodate fixed service quotas and constraints through architecture
        ...
        d) Where applicable, add Citations section for the best practices, recommendations, and risks where applicable by including the best practice ID and heading from the specified lens ("<current_lens>") and specified pillar ("<current_pillar>") under the <kb> section. And every citation within it should be separated by ',' and start on a new line. If there are no citations then return 'N/A' for Citations. 
