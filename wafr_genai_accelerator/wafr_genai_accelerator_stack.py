...
class WafrGenaiAcceleratorStack(Stack):
    ...
    def __init__(
        self, scope: Construct, construct_id: str, tags: dict = None, **kwargs
    ) -> None:
        """
        ...
        super().__init__(
            scope,
            construct_id,
            description="AWS Well-Architected Framework Review (WAFR) Acceleration with Generative AI (GenAI) sample. (uksb-ig1li00ta6)",
            **kwargs,
        )
        ...
        DEAD_LETTER_QUEUE_UNIQUE_NAME = (
            "wafrAcceleratorDeadLetterQueue-" + entryTimestamp
        )
        ...
        wafrRunsTable = dynamodb.TableV2(
            self,
            "review-runs",
        ...
        # Create DynamoDB table for tracking WAFR accelerator runs
        wafrPillarQuestionPromptsTable = dynamodb.TableV2(
            self,
            "wafr-pillar-question-prompts",
        ...
        insertWafrPromptsFunction = _lambda.Function(
            self,
            "insertWAFRPrompts",
        ...
        promptsBucketDeploy = s3deploy.BucketDeployment(
            self,
            "promptsBucketDeploy",
        ...
        vpc = ec2.Vpc(
            self,
            "StreamlitAppVPC-" + entryTimestamp,
            max_azs=2,
            nat_gateways=1,
        ...
        ec2Role = iam.Role(
            self,
            "StreamlitAppRole-" + entryTimestamp,
                ...
                "ec2RolePolicies": iam.PolicyDocument(
                    statements=[
                        ...
                        iam.PolicyStatement(
                            ...
                            resources=[
                                f"arn:aws:ssm:{self.region}:{self.account}:parameter/wafr-accelerator/*"
                            ],
                        ),
                        iam.PolicyStatement(
                            actions=[
        ...
        user_data_script = re.sub(
            r"{{REGION}}", Stack.of(self).region, user_data_script
        )

        ec2_create = ec2.Instance(
            self,
            "StreamlitAppInstance-" + entryTimestamp,
        ...
        alb_security_group = ec2.SecurityGroup(
            self,
            "ALBSecurityGroup-" + entryTimestamp,
            vpc=vpc,
            allow_all_outbound=True,
        ...
        alb = elbv2.ApplicationLoadBalancer(
            self,
            "StreamlitAppALB-" + entryTimestamp,
            vpc=vpc,
        ...
        instance_target = elasticloadbalancingv2_targets.InstanceTarget(
            ec2_create, 8501
        )
        ...
        target_group = elbv2.ApplicationTargetGroup(
            self,
            "StreamlitAppTargetGroup-" + entryTimestamp,
            port=8501,
            protocol=elbv2.ApplicationProtocol.HTTP,
            targets=[instance_target],
            health_check=elbv2.HealthCheck(path="/", port="8501"),
            vpc=vpc,
        )
        # Add listener to ALB
        alb.add_listener(
            "Listener", port=80, default_target_groups=[target_group], open=False
        )
        ...
        CfnOutput(
            self,
            "FrontEnd-EC2-Instance-Id",
            value=EC2_INSTANCE_ID,
            description="Front end UI EC2 instance id created at : "
            + entryTimestampLabel,
        )
        ...
        user_pool = cognito.UserPool(
            self,
            PARAMETER_COGNITO_USER_POOL_NAME,
        ...
        app_client = user_pool.add_client(
            "WafrAcceleratorAppClient-" + entryTimestamp,
            ...
            o_auth=cognito.OAuthSettings(
                ...
                callback_urls=[
                    f"https://{cdn.distribution_domain_name}",
                    "http://localhost:8501",
                ],
            ),
        ...
        uiPage1UpdateParameter = StringParameter(
            self,
            "uiPage1UpdateParameter-" + entryTimestamp,
            parameter_name="/wafr-accelerator/"
            + entryTimestamp
            + "/1_New_WAFR_Review-updated",
            string_value="False",
            description="1_New_WAFR_Review-updated status created at : "
            + entryTimestampLabel,
        )
        ...
        uiPage2UpdateParameter = StringParameter(
            self,
            "uiPage2UpdateParameter-" + entryTimestamp,
            parameter_name="/wafr-accelerator/"
            + entryTimestamp
            + "/2_Existing_WAFR_Reviews-updated",
            string_value="False",
            description="2_Existing_WAFR_Reviews-updated status created at : "
            + entryTimestampLabel,
        )
        ...
        uiSyncFlagParameter = StringParameter(
            self,
            "uiSyncFlagParameter-" + entryTimestamp,
            parameter_name="/wafr-accelerator/"
            + entryTimestamp
            + "/uiSyncInitiatedFlag",
            string_value="False",
        ...
        uiLoginPageParameter = StringParameter(
            self,
            "uiLoginPageParameter-" + entryTimestamp,
            parameter_name="/wafr-accelerator/"
            + entryTimestamp
            + "/uiLoginPageParameter",
            string_value="False",
            description="uiLoginPageParameter-updated status created at : "
            + entryTimestampLabel,
        )
                ...
                "replaceUITokensFunctionRolePolicies": iam.PolicyDocument(
                    statements=[
                        iam.PolicyStatement(
                            ...
                            resources=[f"{wafrUIBucket.bucket_arn}/*"],
                            conditions={
                                "StringEquals": {"aws:ResourceAccount": self.account}
                            },
                            effect=iam.Effect.ALLOW,
        ...
        replaceUITokensFunction = _lambda.Function(
            self,
            "replaceUITokensFunction",
            runtime=_lambda.Runtime.PYTHON_3_12,
                ...
                lambda_events.S3EventSource(
                    ...
                    filters=[
                        s3.NotificationKeyFilter(
                            prefix="tokenized-pages/", suffix=".py"
                        )
                    ],
                )
            ],
        ...
        wafrUIBucketDeploy = s3deploy.BucketDeployment(
            self,
            "uploaduicode",
        ...
        startWafrReviewFunctionRole = iam.Role(
            self,
            "startWafrReviewFunctionRoleLambdaRole",
            assumed_by=iam.ServicePrincipal("lambda.amazonaws.com"),
            managed_policies=[
                iam.ManagedPolicy.from_aws_managed_policy_name(
                    "service-role/AWSLambdaBasicExecutionRole"
                ...
                "startWafrReviewFunctionRolePolicies": iam.PolicyDocument(
                    statements=[
                        ...
                        iam.PolicyStatement(
                            ...
                            resources=[
                                f"arn:aws:states:{self.region}:{self.account}:stateMachine:WAFRReviewStateMachine-{entryTimestamp}"
                            ],
                        ),
                        iam.PolicyStatement(
                            actions=[
        ...
        prepare_wafr_review = _lambda.Function(
            self,
            "prepare_wafr_review",
            runtime=_lambda.Runtime.PYTHON_3_12,
        ...
        extract_document_text = _lambda.Function(
            self,
        ...
        generate_solution_summary = _lambda.Function(
            self,
        ...
        generate_prompts = _lambda.Function(
            self,
            ...
            role=startWafrReviewFunctionRole,
            reserved_concurrent_executions=1,
            environment={"WAFR_REFERENCE_DOCS_BUCKET": WAFR_REFERENCE_DOCS_BUCKET},
        )
        generate_pillar_question_response = _lambda.Function(
            self,
            ...
            role=startWafrReviewFunctionRole,
            reserved_concurrent_executions=1,
            environment={"BEDROCK_SLEEP_DURATION": "60", "BEDROCK_MAX_TRIES": "5"},
        )
        update_review_status = _lambda.Function(
            self,
        ...
        wait_state = sfn.Wait(
            self, "Wait", time=sfn.WaitTime.duration(cdk.Duration.seconds(40))
        )

        # Define the Map state
        map_state = sfn.Map(
            self,
            "Loop through selected pillars",
            max_concurrency=1,
            items_path="$.all_pillar_prompts",
        )

        # Define the iterator chain
        iterator_chain = sfn.Chain.start(wait_state).next(
            generate_pillar_question_response_task
        )
        ...
        wafr_stepmachine_log_group = aws_logs.LogGroup(
            self,
            "WAFRReviewStateMachineLogGroup",
        ...
        startWafrReviewFunction = _lambda.Function(
            self,
            "startWafrReview",
            runtime=_lambda.Runtime.PYTHON_3_12,
            code=_lambda.Code.from_asset(
        ...
        sqs_event_source = lambda_event_source.SqsEventSource(
            wafrAcceleratorQueue, batch_size=1
